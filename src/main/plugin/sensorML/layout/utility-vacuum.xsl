...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  * All elements having no child text (ie. normalize-space return '') and
  ...
  The main function call the vacuum-sensorML mode.
  -->
  <xsl:function name="gn-fn-sensorML:vacuum" as="node()">
    <xsl:param name="metadata" as="node()"/>
    <xsl:for-each select="$metadata/*">
      <xsl:apply-templates mode="vacuum-sensorML"
                           select="."/>
    </xsl:for-each>
  </xsl:function>


  <xsl:function name="gn-fn-sensorML:isElementOrChildEmpty"
                as="xs:boolean">
  ...
  <xsl:template mode="vacuum-sensorML"
                match="@*|node()">
    <xsl:variable name="isElementEmpty"
                  select="gn-fn-sensorML:isElementOrChildEmpty(.)"/>
